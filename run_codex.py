...
def memory_context_for(tags: list[str]) -> str:
    ...
    for tag in list(dict.fromkeys(tags))[:6]:
...
从已预提取的 `data/parsed/{aid}.json` 按需读取 sections，不要 cat 整份 JSON。
sections 包含 abstract, introduction, main_results, techniques, conclusion, full_text；
按定理编号或关键词定位，分节为空时在 full_text 中搜索，重叠内容只读一份。
仅当预提取文件缺失时运行 `tcs-daily extract {aid} > /dev/null` 生成缓存。
...
按 skill 加载一次单篇写作与评分参考。只有需要风格参照时才读范文的相关段落。
...
用本地文件拼接原稿，只对问题段落做局部修改，不逐字重新输出全部稿件。
...
def main() -> None:
    ...
    validation_args = (
        "validate", dt, "--selection", str(sel_path.relative_to(ROOT)), "--require-scores"
    )
    ...
    if run_stage in (None, 3):
        ...
        if can_reuse_report:
            cached_validation = tool(*validation_args)
