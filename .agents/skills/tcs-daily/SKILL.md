...
- 至少使用一个最小的 running example，从问题定义贯穿到主结果和技术思路。
- 专业概念首次出现时，依次说明：它解决什么困难、一个直觉或例子、必要的形式化定义、它在本文中的作用。不要用另一个未解释的术语来解释它。
- 每个符号先交代对象类型与量化范围；每个缩写首次出现时写出全称。
- prior work 必须是可比较的时间线：先前结果的适用范围和定量界、留下的缺口、本文填补的部分。作者年份列表本身不算背景。
- 完稿后只读正文、不展开旁注做一次冷读：读者必须能复述问题输入、目标、主结果、相对已有工作的改进，以及技术为何可能奏效。
